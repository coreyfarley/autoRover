...
**FR_039** - If the battery voltage falls below a defined critical threshold, the system shall log a low-battery warning, activate the red LED, and hold in a low-battery warning state for a minimum configurable duration before initiating a shutdown sequence into sleep mode. All LEDs shall be turned off upon
entering sleep state per FR_034.
